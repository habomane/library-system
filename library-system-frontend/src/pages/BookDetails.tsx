...
function BookDetailsPage() {
  ...
  <main className="container mx-auto">
        <div className="flex flex-col md:flex-row gap-y-3 gap-x-3 justify-around mt-8">
            ...
            <img src={book.image} alt={book.title + " book cover."} className="w-screen h-[70vh]" />
            </div>
            <div className="w-full bg-white rounded-lg shadow-xl ">
                ...
                <div className={`h-5 w-5 ${book.available === true ? "bg-green-500": "bg-red-500"} rounded-xl`}></div>
                    <p className="text-md font-medium">{book.available === true ? "Available": "Not Available"}</p>
                </div>

                <p className="text-lg mt-8 text-gray-600">Author: {book.author}</p>
                <p className="text-lg text-gray-600">Genre: {book.genre}</p>
                <p className="text-lg text-gray-600">Posted By: { book.ownerUserName }</p>
                <p className="text-lg text-gray-600">Location: {formattedAddress}</p>
